...
const avatarBlobId = computed(() => {
    if (isProvider.value && providerPublic.value) return providerPublic.value.logo_blob_id;
...
// --- Avatar & Photo Modal Logic ---
const toggleAvatarDropdown = () => { isAvatarDropdownOpen.value = !isAvatarDropdownOpen.value; };
const closeAvatarDropdown = () => { isAvatarDropdownOpen.value = false; };

const openPhotoModal = (view: PhotoModalView) => {
    closeAvatarDropdown();
    photoModalView.value = view;
    showPhotoModal.value = true;
    if (view === "camera") nextTick(() => startCameraStream());
};
...
const handleDeleteAvatar = async () => {
    ...
    try {
        if (isProvider.value) {
            // Note: Wire up organizationApi.deleteOrganizationLogo if needed
...
const startCameraStream = async () => {
    ...
    } catch (err) { cameraActive.value = false; }
};
...
const handleFileDrop = (event: DragEvent) => {
    if (event.dataTransfer?.files && event.dataTransfer.files[0]) processUploadedFile(event.dataTransfer.files[0]);
};

const processUploadedFile = (file: File) => {
    if (!file.type.startsWith("image/")) { alert("Por favor selecciona un archivo de imagen válido."); return; }
    if (file.size > 3 * 1024 * 1024) { alert("La imagen excede el límite de 3MB."); return; }
    const reader = new FileReader();
    reader.onload = (e) => { if (e.target?.result) prepareImageForAdjust(e.target.result as string); };
...
const saveCroppedAvatar = async () => {
    ...
    canvasElement.value.toBlob(async (blob) => {
        ...
        try {
            ...
            if (isProvider.value) {
                // await organizationApi.uploadOrganizationLogo(contextStore.activeOrganizationId!, file);
    ...
    <div class="profile-container">
        ...
        <div class="card profile-card">
            <div class="avatar-container">
                <div class="avatar">
                    <ProfileAvatar v-if="!isProvider" :blob-id="avatarBlobId" :alt="displayName" />
                    <ProviderLogo v-else :blob-id="avatarBlobId" :alt="displayName" />
                </div>
                <button type="button" class="edit-avatar-btn" aria-label="Cambiar foto" @click="toggleAvatarDropdown">
                    <i class="fa-solid fa-camera"></i>
                </button>
                <div v-if="isAvatarDropdownOpen" class="avatar-dropdown">
                    <button class="dropdown-item" @click="openPhotoModal('choice')">
                        <i class="fa-solid fa-rotate"></i> Cambiar Foto
                    </button>
                    <button v-if="avatarBlobId" class="dropdown-item" @click="() => { closeAvatarDropdown(); showViewPhotoModal = true; }">
                        <i class="fa-regular fa-image"></i> Ver foto
                    </button>
                    <div v-if="avatarBlobId" class="dropdown-divider"></div>
                    <button v-if="avatarBlobId" class="dropdown-item danger" @click="() => { closeAvatarDropdown(); showDeletePhotoModal = true; }">
        ...
        <!-- Modals (Retained from original ProfileView for Avatar & Buyer Editing) -->
        <BaseModal v-model="showEditProfileModal" max-width="620px" :show-close-button="false" @close="showEditProfileModal = false">
            ...
            <div class="edit-photo-section">
                ...
                <div class="edit-photo-info">
                    ...
                    <button type="button" class="btn-change-photo" @click="() => { showEditProfileModal = false; openPhotoModal('choice'); }">
                        <i class="fa-solid fa-rotate"></i> Cambiar Foto
                    </button>
                </div>
            </div>
        ...
        <BaseModal v-model="showViewPhotoModal" max-width="400px" @close="showViewPhotoModal = false">
            <div class="view-photo-container">
                <ProfileAvatar v-if="!isProvider" :blob-id="avatarBlobId" alt="Foto ampliada" />
                <ProviderLogo v-else :blob-id="avatarBlobId" alt="Logo ampliado" />
            </div>
        </BaseModal>

        <ConfirmModal v-model="showDeletePhotoModal" title="¿Deseas eliminar tu foto de perfil?" confirm-text="Eliminar" cancel-text="Cancelar" icon="fa-regular fa-trash-can" icon-variant="orange" @confirm="handleDeleteAvatar" @cancel="showDeletePhotoModal = false" />

        <BaseModal v-model="showPhotoModal" max-width="480px" :show-close-button="false" @close="closePhotoModal">
            <template #header>
                <div class="modal-nav-header">
                    <button class="btn-back-nav" @click="closePhotoModal"><i class="fa-solid fa-arrow-left"></i></button>
                    <h3 class="modal-heading">{{ photoModalView === "camera" ? "Tomar Foto" : photoModalView === "adjust" ? "Ajustar Foto" : "Cambiar Foto" }}</h3>
                    <button class="btn-close-circle" @click="closePhotoModal"><i class="fa-solid fa-xmark"></i></button>
                </div>
            </template>
            ...
            <div v-if="photoModalView === 'upload'" class="upload-view">
                <div class="drag-drop-zone" @dragover.prevent @drop.prevent="handleFileDrop">
                    ...
                    <label class="btn-outline">Seleccionar archivo<input type="file" accept="image/png, image/jpeg" style="display: none" @change="handleFileSelected" /></label>
                </div>
                <div class="upload-info"><p>Formatos: JPG, PNG</p><p>Tamaño máx: 3MB</p></div>
                <div class="upload-controls"><button class="btn-secondary" @click="photoModalView = 'choice'">Atrás</button></div>
            </div>
            <div v-if="photoModalView === 'camera'" class="camera-view">
                ...
                <div class="camera-controls">
                    ...
                    <button class="btn-icon" @click="() => { stopCameraStream(); photoModalView = 'upload'; }"><i class="fa-solid fa-image"></i></button>
                </div>
            </div>
            <div v-if="photoModalView === 'adjust'" class="adjust-view">
                <p class="adjust-instruction">Arrastra la imagen para centrarla.</p>
                <div class="adjust-container">
                    <canvas ref="canvasElement" class="photo-canvas" @mousedown="(e) => startCanvasDrag(e.clientX, e.clientY)" @mousemove="(e) => moveCanvasDrag(e.clientX, e.clientY)" @mouseup="endCanvasDrag" @mouseleave="endCanvasDrag" @touchstart="(e) => startCanvasDrag(e.touches[0].clientX, e.touches[0].clientY)" @touchmove="(e) => moveCanvasDrag(e.touches[0].clientX, e.touches[0].clientY)" @touchend="endCanvasDrag"></canvas>
...
/* Modals & Forms (Condensed for brevity, identical to original ProfileView) */
...
.adjust-controls-bottom, .upload-controls { display: flex; justify-content: space-between; }
