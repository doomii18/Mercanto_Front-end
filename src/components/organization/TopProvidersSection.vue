...
function deriveMockProviderData(id: string) {
  const seed = hashSeed(id);
  const rating = Number((4.0 + (seed % 10) * 0.1).toFixed(1));
  const locationText = LOCATIONS[seed % LOCATIONS.length];
  const isVerified = (seed % 10) < 9;

  return { rating, locationText, isVerified };
}

async function loadTopProviders() {
  isLoading.value = true;
  try {
    const res = await organizationApi.getOrganizations({ limit: 3, offset: 0 });
    providers.value = res.data.map((prov) => {
      const mock = deriveMockProviderData(prov.id);
      return {
        id: prov.id,
        name: prov.company_name,
        logoBlobId: prov.logo_blob_id ?? null,
        rating: mock.rating,
        locationText: mock.locationText,
...
<template>
  <section id="proveedores" class="top-providers container">
    ...
    <div v-if="isLoading" class="providers-grid">
      <div v-for="n in 3" :key="n" class="provider-card skeleton-card">
    ...
    <div v-else class="providers-grid">
      ...
      >
        ...
        <div class="provider-rating">
          <span class="score">{{ provider.rating.toFixed(1) }}</span>
          <i class="fa-solid fa-star"></i>
        </div>

        <p class="provider-location">{{ provider.locationText }}</p>
